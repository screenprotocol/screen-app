...
const AccountInfo = () => {
    const { sdk, safe } = useSafeConnector()
        ...
        const getSafeBalance = async () => {
            ...
            setSafeName(
                addressBook.reduce((result, address) => {
                    return address.address === safe.safeAddress
                        ? address.name
                        : result
                }, ''),
            )
    ...
    return (
        <Stack sx={{ m: 4 }} direction="row" display="flex" alignItems="center">
            ...
            <Stack>
                {safe.chainId === 5 ? (
                    <ChainIndicator
                        style={{
                            backgroundColor: 'rgb(77, 153, 235)',
